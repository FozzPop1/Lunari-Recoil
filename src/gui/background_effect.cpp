...
void RenderStarBackground(ImDrawList* dl, ImVec2 pos, ImVec2 size, int windowId)
{
    ...
    // CLEAN deep space background (no purple tint)
    dl->AddRectFilled(
        pos,
    ...
    auto Nebula = [&](float nx, float ny, float nr, float r, float g, float b, float a)
    {
        for (int i = 5; i >= 1; i--) {
            float t = (float)i / 5.f;
            float alpha = a * (1.f - t) * 0.10f;

            dl->AddCircleFilled(
                ImVec2(pos.x + nx, pos.y + ny),
                nr * t,
                ImGui::ColorConvertFloat4ToU32(ImVec4(r, g, b, alpha)),
                24
            );
        }
    };

    // subtle CLEAN nebula (low saturation, not neon)
    ...
    for (auto& s : w.stars)
    {
        ...
        float twinkle =
            0.5f + 0.5f *
            sinf(lt * (1.2f + s.alphaSpeed * 70.f) + s.twinklePhase);

        float finalAlpha = twinkle * (0.25f + s.layer * 0.22f);

        // CLEAN star palette (white → soft blue)
        ...
        // very subtle trail (no harsh glow)
        for (int i = 0; i < 6; i++) {
            ...
            float a = finalAlpha * t1 * 0.4f;

            dl->AddLine(
                p0,
                p1,
                ImGui::ColorConvertFloat4ToU32(ImVec4(r * 0.7f, g * 0.8f, b, a)),
                s.radius * 0.35f
            );
        }

        // minimal glow (clean look)
        dl->AddCircleFilled(
            starPos,
            s.radius * 1.8f,
            ImGui::ColorConvertFloat4ToU32(ImVec4(r, g, b, finalAlpha * 0.15f)),
            10
        );

        // core star (kept crisp)
        dl->AddCircleFilled(
            starPos,
            s.radius,
            ImGui::ColorConvertFloat4ToU32(ImVec4(1.f, 1.f, 1.f, finalAlpha)),
            8
        );
    }
